...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.value_preds_mix = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.returns = np.zeros_like(self.value_preds)
        self.returns_mix = np.zeros_like(self.value_preds_mix)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None, value_preds_mix=None, r_in=None):
        """
        ...
        self.value_preds_mix[self.step] = value_preds_mix.copy()
        self.rewards[self.step] = rewards.copy()
        self.rewards_mix[self.step] = rewards.copy() + 0.01 * r_in.copy()
    ...
    def compute_returns(self, next_value, next_value_mix, value_normalizer=None):
        """
        ...
        if self._use_proper_time_limits:
            ...
            if self._use_gae:
                self.value_preds[-1] = next_value
                self.value_preds_mix[-1] = next_value_mix
                gae = 0
                gae_mix = 0
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        if self.algo == "mat" or self.algo == "mat_dec":
                            ...
                            delta_mix = self.rewards_mix[step] + self.gamma * value_normalizer.denormalize(
                                self.value_preds_mix[step + 1]) * self.masks[step + 1] \
                                    - value_normalizer.denormalize(self.value_preds_mix[step])
                            gae_mix = delta_mix + self.gamma * self.gae_lambda * self.masks[step + 1] * gae_mix
                            self.returns_mix[step] = gae_mix + value_normalizer.denormalize(self.value_preds_mix[step])
                        else:
                            delta = self.rewards[step] + self.gamma * value_normalizer.denormalize(
                                self.value_preds[step + 1]) * self.masks[step + 1] \
                                    - value_normalizer.denormalize(self.value_preds[step])
                        ...
                        if self.algo == "mat" or self.algo == "mat_dec":
                            ...
                            delta_mix = self.rewards_mix[step] + self.gamma * self.value_preds_mix[step + 1] * \
                                    self.masks[step + 1] - self.value_preds_mix[step]
                            gae_mix = delta_mix + self.gamma * self.gae_lambda * self.masks[step + 1] * gae_mix
                            self.returns_mix[step] = gae_mix + self.value_preds_mix[step] 
    ...
    def naive_recurrent_generator(self, advantages, advantages_mix, num_mini_batch):
        """
        ...
        value_preds_mix = self.value_preds_mix.reshape(-1, batch_size, 1)
        returns = self.returns.reshape(-1, batch_size, 1)
        returns_mix = self.returns_mix.reshape(-1, batch_size, 1)
        ...
        advantages_mix = advantages_mix.reshape(-1, batch_size, 1)
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                value_preds_mix_batch.append(value_preds_mix[:-1, ind])
                return_batch.append(returns[:-1, ind])
                return_mix_batch.append(returns_mix[:-1, ind])
                masks_batch.append(masks[:-1, ind])
                active_masks_batch.append(active_masks[:-1, ind])
                old_action_log_probs_batch.append(action_log_probs[:, ind])
                adv_targ.append(advantages[:, ind])
                adv_mix_targ.append(advantages_mix[:, ind])
                rewards_batch.append(rewards[:,ind])
            ...
            value_preds_mix_batch = np.stack(value_preds_mix_batch, 1)
            return_batch = np.stack(return_batch, 1)
            return_mix_batch = np.stack(return_mix_batch, 1)
            masks_batch = np.stack(masks_batch, 1)
            active_masks_batch = np.stack(active_masks_batch, 1)
            old_action_log_probs_batch = np.stack(old_action_log_probs_batch, 1)
            adv_targ = np.stack(adv_targ, 1)
            adv_mix_targ = np.stack(adv_mix_targ, 1)
            ...
            value_preds_mix_batch = _flatten(T, N, value_preds_mix_batch)
            return_batch = _flatten(T, N, return_batch)
            return_mix_batch = _flatten(T, N, return_mix_batch)
            masks_batch = _flatten(T, N, masks_batch)
            active_masks_batch = _flatten(T, N, active_masks_batch)
            old_action_log_probs_batch = _flatten(T, N, old_action_log_probs_batch)
            adv_targ = _flatten(T, N, adv_targ)
            adv_mix_targ = _flatten(T, N, adv_mix_targ)
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                  ...
                  adv_targ, adv_mix_targ, available_actions_batch, rewards_batch

    def recurrent_generator(self, advantages, advantages_mix, num_mini_batch, data_chunk_length):
        """
        ...
        print('self.share_obs:',self.share_obs.shape)
        print('batch_size:',batch_size)
        print('data_chunk_length:',data_chunk_length)
        print('data_chunks:',data_chunks)
        print('num_mini_batch:',num_mini_batch)
        print('mini_batch_size:',mini_batch_size)
        print('self.actions:', self.actions.shape)
        print('self.action_log_probs:',self.action_log_probs.shape)
        print('self.share_obs:',self.share_obs.shape)
        print('share_obs.transpose:',self.share_obs.transpose(1,2,0,3).shape)
        ...
        if advantages_mix is not None:
            advantages_mix = _cast(advantages_mix)
        value_preds = _cast(self.value_preds[:-1])
        value_preds_mix = _cast(self.value_preds_mix[:-1])
        returns = _cast(self.returns[:-1])
        returns_mix = _cast(self.returns_mix[:-1])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                value_preds_mix_batch.append(value_preds_mix[ind:ind + data_chunk_length])
                return_batch.append(returns[ind:ind + data_chunk_length])
                return_mix_batch.append(returns_mix[ind:ind + data_chunk_length])
                masks_batch.append(masks[ind:ind + data_chunk_length])
                active_masks_batch.append(active_masks[ind:ind + data_chunk_length])
                old_action_log_probs_batch.append(action_log_probs[ind:ind + data_chunk_length])
                adv_targ.append(advantages[ind:ind + data_chunk_length])
                adv_mix_targ.append(advantages_mix[ind:ind + data_chunk_length])
            ...
            print('L:',L)
            print('N:',N)
            print('share_obs_batch:', len(share_obs_batch))
            print('obs_batch:', len(obs_batch))
            print('share_obs_batch[0]:',share_obs_batch[0].shape)
            print('obs_batch[0]:', obs_batch[0].shape)
            ...
            value_preds_mix_batch = np.stack(value_preds_mix_batch, axis=1)
            return_batch = np.stack(return_batch, axis=1)
            return_mix_batch = np.stack(return_mix_batch, axis=1)
            masks_batch = np.stack(masks_batch, axis=1)
            active_masks_batch = np.stack(active_masks_batch, axis=1)
            old_action_log_probs_batch = np.stack(old_action_log_probs_batch, axis=1)
            adv_targ = np.stack(adv_targ, axis=1)
            adv_mix_targ = np.stack(adv_mix_targ, axis=1)

            # States is just a (N, -1) from_numpy
            rnn_states_batch = np.stack(rnn_states_batch).reshape(N, *self.rnn_states.shape[3:])
            rnn_states_critic_batch = np.stack(rnn_states_critic_batch).reshape(N, *self.rnn_states_critic.shape[3:])

            print('value_preds_batch:', value_preds_batch.shape)
            ...
            value_preds_mix_batch = _flatten(L, N, value_preds_mix_batch)
            return_batch = _flatten(L, N, return_batch)
            return_mix_batch = _flatten(L, N, return_mix_batch)
            masks_batch = _flatten(L, N, masks_batch)
            active_masks_batch = _flatten(L, N, active_masks_batch)
            old_action_log_probs_batch = _flatten(L, N, old_action_log_probs_batch)
            adv_targ = _flatten(L, N, adv_targ)
            adv_mix_targ = _flatten(L, N, adv_mix_targ)
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                  value_preds_batch, value_preds_mix_batch, return_batch, return_mix_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
                  adv_targ, adv_mix_targ, available_actions_batch
