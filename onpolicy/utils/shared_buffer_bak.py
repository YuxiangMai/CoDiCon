...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.returns_mix = np.zeros_like(self.value_preds)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, r_in=None, bad_masks=None, active_masks=None, available_actions=None, value_preds_mix=None):
        """
        ...
        if value_preds_mix is not None:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        ...
                        if self.algo == "mat" or self.algo == "mat_dec":
                            ...
                            delta_mix = self.rewards_mix[step] + self.gamma * self.value_preds[step + 1] * \
                                    self.masks[step + 1] - self.value_preds[step]
                            gae_mix = delta_mix + self.gamma * self.gae_lambda * self.masks[step + 1] * gae_mix
                            self.returns_mix[step] = gae_mix + self.value_preds[step] 
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length, advantages_mix=None):
        """
