...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        elif self.algorithm_name == "cc":
            from onpolicy.algorithms.cc.cc_trainer import CC_MAPPO as TrainAlgo
            from onpolicy.algorithms.cc.algorithm.cc_policy import CCPolicy as Policy
        else:
        ...
        # policy network
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec" or self.algorithm_name == "cc":
    ...
    def compute(self):
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            next_values, next_values_mix = self.trainer.policy.get_values(np.concatenate(self.buffer.share_obs[-1]),
                                                        np.concatenate(self.buffer.rnn_states_critic[-1]),
                                                        np.concatenate(self.buffer.masks[-1]))
        next_values = np.array(np.split(_t2n(next_values), self.n_rollout_threads))
        next_values_mix = np.array(np.split(_t2n(next_values_mix), self.n_rollout_threads))
        # print('next_values:', next_values.shape)
        # print('next_values_mix:', next_values_mix.shape)
        self.buffer.compute_returns(next_values, next_values_mix, self.trainer.value_normalizer)
